...
Now the \rubik{} is permuted with the initialized move sequence and hereafter as new test if performed to check if it is in \m{H}.
If it is in \m{H} now the same method from before \vr{solveFromH} will be called.
If the \rubik{} is not in \m{H} the method will permute the \rubik{} with the inverse move sequence but only the last move.
It then tests if all the moves in \m{A} has been used on the last move.
If the last move can not be incremented then it will try to increment the second last move, and so on.
When a move has been incremented the \rubik{} will be permuted with this new move sequence and the test will run again and a new move will be incremeneted untill the \rubik{} is in \m{H}.
After all the moves in the move sequence has been incremented, then the method will increase the move sequence length and initialize a new move sequence.
In this instance the new move sequence would be: \m{U D U D U F}.
