...
The size of a lookup table containing every move sequences to solve inside $H$ would be: $987$ GB, which exceeds the average memory on todays computer \cite{averageRAM} \cite{maxRAM2}.
...
The optimal solver's approach to solving the \rubik{} is trying every move sequence not tried so far and if the cube is inside $H$ it will start using only $A$ moves. Every time a shorter move sequence is found it is saved.
