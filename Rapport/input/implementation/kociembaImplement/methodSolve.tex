...
\label{sec:kSolve}
The method named \vr{solve} (see code snippet \ref{src:kociemba} for a key point) acts like a main routine for the solving mechanism of Kociemba's optimal solver. By calling this method kociemba's optimal solver will be applied onto the \rubik{}. 
...
\vr{UnableToIncreaseMoveSequenceException}. The \vr{solve} method catches this exception and increments \vr{d}. When \vr{d} is incremented to \vr{l} the method will return the \vr{result} array which contains the shortest move sequence to the given \rubik{}.  
\begin{comment}
