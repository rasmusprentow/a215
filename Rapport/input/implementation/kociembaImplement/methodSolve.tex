...
\label{sec:kSolve}
The method named \vr{solve} (see code snippet \ref{src:kociemba}) acts like a main routine for the solving mechanism. By calling this method kociemba's algorithm will be applied onto the \rubik{}. 
...
The \vr{solve} method starts with the \vr{solveFromH} method to test if the cube is in \m{H}. The method \vr{solveFromH} is called with a parameter, which determines how deep it will search. If the \rubik{} is in \m{H} then the method will start solving it with \m{A} moves.
