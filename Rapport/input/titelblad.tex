% Dette er LaTeX-versionen af titelbladet for tek-nat-basis-rapporter 2004 efterår
% Filen kr¾ver:
% Universitetets logo:  aau-logo.png (for LaTeX) eller aau-logo.ps (for LaTeX)
% Synopsis: En fil ved navn synopsis.tex

% Udarbejdet af: Hans HŸttel (hans@cs.auc.dk) 21. maj 2003
% Rettet af Morten Christophersen (mortench@tnb.aau.dk) 30. nov 2004(ændret til nyt design 2004 efterår)

%\documentclass[11pt]{article}
%\ifx\pdfoutput\undefined 
%\usepackage[dvips]{graphicx}
%\else
%\usepackage[pdftex]{graphicx} 
%\usepackage{type1cm} \fi
%    \usepackage[ansinew]{inputenc}
%    \usepackage{a4}

%\begin{document} 
\thispagestyle{empty}
\begin{titlepage}
\begin{nopagebreak}
{\samepage
\raisebox{11mm}{
\begin{tabular}{r}
\parbox{\textwidth}{
\includegraphics[height=1.2cm]{input/pics/aaulogo.png}
\hfill \parbox{4.9cm}{\begin{tabular}{l}
{\sf\small \textbf{Det Teknisk-Naturvidenskabelige Basis{\aa}r }}\\
{\sf\small  \textbf{Software}} \\
{\sf\small Strandvejen 12-14} \\
{\sf\small Telephone 96 35 97 31} \\
{\sf\small Fax 98 13 63 93} \\
{\sf\small http://tnb.aau.dk}
\end{tabular}}}
\\
\end{tabular}}

\begin{tabular}{cc}
\parbox{7cm}{
\begin{description}

\item {\bf Title:} \\
\rubik{}
\item {\bf Tema:} \\
TBD
\end{description}

\parbox{8cm}{

\begin{description}
\item {\bf Project period :}\\
   P2, for{\aa}rssemesteret 2010
  \hspace{2cm}
\item {\bf Projektgruppe:}\\
  A215 
  \hspace{2cm}
\item {\bf Participants:}\\
\rule[-0.1cm]{5cm}{0.01cm} \\
Mikael Midtgaard \\
\rule[-0.1cm]{5cm}{0.01cm} \\
Jens Mohr Mortensen \\
\rule[-0.1cm]{5cm}{0.01cm} \\
Christoffer Ilsø Vinther \\
\rule[-0.1cm]{5cm}{0.01cm} \\
Rasmus Veiergang Prentow \\
\rule[-0.1cm]{5cm}{0.01cm} \\
Dan Stenholt Møller \\
\rule[-0.1cm]{5cm}{0.01cm} \\
Mikkel Bach Klovnhøj \\
\rule[-0.1cm]{5cm}{0.01cm} \\
Alex Bondo Andersen
  \hspace{2cm}
\item {\bf Advisors:}\\
 Anders Bruun \\
 Heather Baca-Greif
\end{description}
}
\begin{description}
\item {\bf Page count:} \pageref{LastPage}
\item {\bf Appendice count and type:} TBD
\item {\bf Finished:} 27/5--2010
\end{description}
\vfill } &
\parbox{7cm}{
  \vspace{.15cm}
  \hfill 
  \begin{tabular}{l}
  {\bf Synopsis:}\bigskip \\
  \fbox{
    \parbox{6.5cm}{\bigskip
     {\vfill{\small \input{input/synopsis}
     \bigskip}}
     }}
   \end{tabular}}
\end{tabular}}
\\ \\
\noindent{\footnotesize\emph{The content of this report is open to everyone, but publishing (with cites) is only allowed after agreed upon by the writers.}}
\end{nopagebreak}
\end{titlepage}
\pagebreak
\thispagestyle{empty}
\begin{titlepage}

\end{titlepage}
\pagebreak
%\end{document}
