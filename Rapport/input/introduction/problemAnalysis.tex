...
<<<<<<< .mineThe ultimate goal for the twist-wise efficiency community is to find the God's algorithm, which is the algorithm that solves the cube in the absolute least amount of twists from any given position. A part of finding the God's algorithm is to find the amount of moves needed to perform it. The lower bound of the \rubik{} is the minimum number of twists that the most efficient algorithm at the moment can perform. The upper bound is the least number of twists required to solve the cube in the currently known worst case scenario. It is interesting to study this part of the \rubik{} community because it is currently moving, proving new upper bounds.
...
The upper bound of the \rubik{} is the maximum number of twists need to solve an arbitrary \rubik{}.
The lower bound is the least number of twists required to solve the cube in the currently known worst case scenario. It is interesting to study this part of the \rubik{} community because it is currently moving, proving new upper bounds.
>>>>>>> .theirs%The lowest proven upper bound to date was published by Thomas Rokicki in 2009 \cite{rokicki09}.
...
\begin{itemize}
	\item Which solving algorithms exist and which is the most efficient with respect to the number of twists?
\end{itemize}
