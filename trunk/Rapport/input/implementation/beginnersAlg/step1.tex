...
The program does this by applying a \m{U} \twist{} and checks if the edge \cpiece{} is above it's correct position.
...
If the edge \cpiece{} is oriented incorrectly \vr{algorithm 1} is performed. If more edge  \cpiece{}s needs to be positioned or oriented correctly the program will continue with the same method until the cross is finished in which case the program moves on to the next step. See figure \ref{fig:FlCrossFlow}.
