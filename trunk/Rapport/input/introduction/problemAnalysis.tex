...
The ultimate goal for the twist-wise efficiency community is to find the God's algorithm, which is the algorithm that solves the cube in the absolute least amount of twists from any given position. A part of finding the God's algorithm is to find the amount of moves need to perform it. The upper bound of the \rubik{} is the minimum number of twists that the most efficient algorithm at the moment can perform. The lower bound is the least number of twists required to solve the cube in the currently known worst case scenario. It is interesting to study this part of the \rubik{} community because it is currently moving, proving new upper bounds.
